...
BasCoD_single=function(X_0,X_j,R_0,R_j,embedding_0,embedding_j,eps=0.3){
  p=dim(X_0)[2]
  ...
  Gammajhat=apply(Gammajhat,2,function(x)x/norm(x,"2"))    
    ...
    pval=pchisq(sum(-2*log(pnorm((atanh(rho)-atanh(1-eps))*sqrt(p-3)))),lower.tail = FALSE,df=2*R_j)
    
 
  
print(paste0("BasCoD p-value is : ",pval))
return(list(corr_vals=rho,pvalue=pval))                    
}
